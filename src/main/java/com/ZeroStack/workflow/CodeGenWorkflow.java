...
import com.ZeroStack.ai.model.message.WorkflowProgressMessage;
...
import com.ZeroStack.workflow.node.*;
...
public class CodeGenWorkflow {
    ...
    /**
     * 执行工作流（Flux 流式输出版本）
     * 输出格式为 JSON 的 WorkflowProgressMessage，与前端消息体系兼容
     */
            ...
            Thread.startVirtualThread(() -> {
                try {
                    ...
                    WorkflowContext initialContext = WorkflowContext.builder()
                            ...
                            .originalPrompt(originalPrompt)
                            .currentStep("初始化")
                            .streamMessageConsumer(sink::next)
                            .build();

                    // 推送工作流启动消息
                    sink.next(JSONUtil.toJsonStr(new WorkflowProgressMessage(
                            "初始化", 0, "开始执行 Agent 代码生成工作流"
                    )));
                    ...
                    for (NodeOutput<MessagesState<String>> step : workflow.stream(
                        ...
                        WorkflowContext currentContext = WorkflowContext.getContext(step.state());
                        if (currentContext != null) {
                            String stepName = currentContext.getCurrentStep();
                            log.info("--- 第 {} 步完成: {} ---", stepCounter, stepName);
                            // 推送步骤完成进度消息
                            sink.next(JSONUtil.toJsonStr(new WorkflowProgressMessage(
                                    stepName, stepCounter, "步骤「" + stepName + "」已完成"
                            )));
                        }
                        stepCounter++;
                    }
                    ...
                    // 推送工作流完成消息
                    sink.next(JSONUtil.toJsonStr(new WorkflowProgressMessage(
                            "完成", stepCounter, "Agent 代码生成工作流执行完成！"
                    )));
                    log.info("代码生成工作流执行完成！");
                    sink.complete();
                } catch (Exception e) {
